...
| 日期 | 決策 | 來源 |
|------|------|------|
| 2026-06-20 | **v2.0.4 USER-MANDATED 17-trade opt params LOCK-IN**：經 SL_ATR_Mult 三組 A/B (1.5/2.0/2.5) 對照，SL=2.5 全勝（PF 9.43 / Sharpe 0.49 / Sortino 0.91 / MaxDD -6.74% / WR 76.5%）。用戶**改變**先前「v2.0.1 .pla 維持 neutral framework」directive，明確要求把 17-trade best params lock-in .pla defaults。**跟先前 RETIRED v2.0.2 lock-in (68f26e1) 區分**：那是 v2.0.1 結構 + 27-trade lock-in（用戶 revert），這是 v2.0.3 結構（含 Secular Bull Filter）+ 17-trade lock-in（user explicit mandate）。**LOCKED params (vs v2.0.3 default)**: H60_RSI_Threshold 65→70, ATR_Spike_Mult 1.3→1.0, Pullback_Min_Pct 0.3→0.5, TP_Pct 0.6→1.0, SL_ATR_Mult 3→2.5, Max_Bars_TimeStop 18→27 (135min), Entry_Cutoff_Time 1325→1230, HighConv_Threshold 2.5→5.0. **STRUCTURE unchanged**: 60M Tier 1 / 5M Tier 2 / Engine SL + Frozen SL / Holiday / Settlement / Daily Data3 Secular Filter 全保留. verify 111→113 PASS. | 本 session（用戶 explicit mandate after 三組 A/B 對照）|
