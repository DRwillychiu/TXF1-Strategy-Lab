...
# 今天有兩次數字對不上都是把前者當成後者。
DRAWN = ('P29 P49 P50 P51 P52 P53 P54 P30 P61 P18 P57 P17 P28 P64 P55 '
...
# 2026-09-01: 十一種編碼進 IND_S16S_ELEVEN (Build 260913)，
# 與 3x3 格四種一起等 MC12 驗收。
PENDING = ('P08 P10 P11 P22 '
           'P33 P35 P37 P13 P14 P41 P42 P43 P45 P47 P48').split()
# 邏輯從未審查（2026-08-25 退回的那批裡還沒處理的）
# 2026-09-01: P17 離開這裡 -- 它一直被畫著，只是掛了 P68 的號。
# 2026-08-31 晚: 十二種走完流程，P39 化約掉，剩十一種已定義未編碼。
UNREVIEWED = []
REVIEWED = []
...
# 結構性排除
# P68 於 2026-09-01 加入: 雙頂四變體依「尖 vs 圓」細分，那是曲率門檻。
# 它從 260870 起被誤標在編碼的等高雙頂上, 正名後歸位到結構性排除。
# P39 於 2026-08-31 晚加入: 三下降峰就是頭頭降原語, 5,280 對 5,280 逐字
# 相同, 且對低點不設條件, 骨架已在圖上。比照 P24/P25 不另外編碼。
EXCLUDED = 'P31 P62 P67 P07 P58 P69 P70 P71 P68 P39'.split()
...
BUCKETS = [
    ...
    ('ready', '已編碼，尚未 MC12 驗收', PENDING,
     '<b>IND_S16S_P29</b> Build 260910 把 3x3 格補滿：P11 上升楔形 1,919、'
     'P10 對稱三角 463、P08 下降三角 2,065、P22 箱型 3,063。'
     '同一版把 P53／P54 的「平」從<b>完全相等</b>改成<b>收斂</b>'
     '（散布 &lt; 對邊移動距離）—— 完全相等是全檔唯一尺度相依的測試，'
     '八年只有 7 個和 8 個且 2026 掛零。全等的那些保留成子標籤。'
     '<br><br><b>IND_S16S_ELEVEN</b> Build 260913（<b>新檔</b>，第二條副圖色帶）'
     '把最後十一種編碼：P41 WR4 82,258、P42 WR7 44,455、P35 結構破壞 24,132、'
     'P33 上衝回落 18,003、P37 性格轉變 12,675、P13 空方旗形 12,041、'
     'P43 關鍵反轉 8,308、P14 空方三角旗 5,883、P47 號角頂 5,020、'
     'P48 管狀頂 4,973、P45 破前時段低 1,463 —— <b>合計 219,211</b>，'
     '比收斂六種還密，所以走色帶不走標籤。十一列先分骨架再分密度。',
     '<b>跑兩份 MC12 驗收</b>。260910 的三道護欄必須不動'
     '（P29 202／P51 1670／P52 1716），全等子集必須仍是 7 與 8；'
     '260913 應印 <code>bars= 421506  pole_overflow= 0</code> 與上列十一個數字'
     '（<b>筆電</b> csv，桌機請對桌機自己重跑的）。'),
    ('ready', '已完成完整流程，尚未編碼', REVIEWED,
     '2026-08-31 晚。十二種進來、<b>P39 三下降峰在寫定義前就被化約</b>'
     '（＝頭頭降原語，5,280 對 5,280 逐字相同）。'
     '其餘十一種各有零參數定義、K 棒示意圖與真實案例。'
     '四個裝置：<b>排名</b>（Crabel N=4/7，借用常數）、'
     '<b>上影 &gt; 實體</b>（把「長」從門檻改成比較）、'
     '<b>差距 &lt; 影線</b>（容差由 K 棒自己給）、'
     '<b>跑到分出勝負</b>（旗形因此沒有根數）。'
     '鏡像倍率 0.78–1.02x，<b>沒有一個偏好空方</b>；'
     '三個過 Bonferroni 門檻但方向全反，那是台指偏多漂移不是型態的邊；'
     'P41/P42 的鏡像測試<b>結構上是空的</b>（振幅在鏡射下不變）。',
     '<b>編碼上圖。</b>依 08-31 落地標準，「沒有方向」不是不畫的理由 —— '
     '方向的判決屬於訊號層。P41/P42 母體 8.2 萬與 4.4 萬，'
     '應比照收斂六種放副圖色帶。'),
...
.note h3{margin:0 0 8px;font-size:15px;color:var(--ink)}
    ...
    w('<div class="note"><h3>那個 dn／up 的量測放在哪裡</h3>'
      '2026-08-31 量到 P18 破位往下 50.43%、P57 54.12%、P17 53.59%，'
