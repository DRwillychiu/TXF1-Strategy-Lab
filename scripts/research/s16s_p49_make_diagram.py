...
<div class="card">
  <h2>十、MC12 逐筆對照 — 202 / 202 全數相符</h2>
  <p class="note">2026-08-26 第二次跑，涵蓋 2019-01 至 2026-08 全期。
  指標的 <code>P29 FORM</code> 事件與 Python 參考實作<b>逐年、逐筆比對</b>：</p>
  <table>
    <tr><th>年</th><th class="n">MC12</th><th class="n">Python</th><th></th></tr>
    <tr><td>2019</td><td class="n">8</td><td class="n">8</td><td class="ok">相符</td></tr>
    <tr><td>2020</td><td class="n">15</td><td class="n">15</td><td class="ok">相符</td></tr>
    <tr><td>2021</td><td class="n">18</td><td class="n">18</td><td class="ok">相符</td></tr>
    <tr><td>2022</td><td class="n">31</td><td class="n">31</td><td class="ok">相符</td></tr>
    <tr><td>2023</td><td class="n">24</td><td class="n">24</td><td class="ok">相符</td></tr>
    <tr><td>2024</td><td class="n">31</td><td class="n">31</td><td class="ok">相符</td></tr>
    <tr><td>2025</td><td class="n">42</td><td class="n">42</td><td class="ok">相符</td></tr>
    <tr><td>2026</td><td class="n">33</td><td class="n">33</td><td class="ok">相符</td></tr>
    <tr><td><b>合計</b></td><td class="n"><b>202</b></td><td class="n"><b>202</b></td>
        <td class="ok"><b>零漏抓、零誤報</b></td></tr>
  </table>
  <p class="note">抽 10 筆逐欄位比對（<code>kind</code> / <code>span</code> /
  <code>s1</code> / <code>open</code>）：<b>10 / 10 完全相符</b>。
  <code>kind</code> 分布 P49 112 ／ P50 90，與 Python 一致。</p>

  <h3 style="margin-top:20px">前一輪的兩個 bug 都確認修好</h3>
  <table>
    <tr><th>#</th><th>症狀</th><th>結果</th></tr>
    <tr><td>1</td><td><code>s1</code> 恆為 0</td>
        <td class="ok">已修，現在 0-5 有分布</td></tr>
    <tr><td>2</td><td>外包棒漏抓 <code>20250804 0945</code>、<code>20251023 0250</code></td>
        <td class="ok">兩筆都出現了</td></tr>
  </table>

  <h3 style="margin-top:20px">★ 這次對照抓到的第三個問題 — 錯的是參考實作</h3>
  <p class="note">修好前兩個之後，<code>s1</code> 仍系統性比 Python 少 1
  —— 但只在某些筆。查出成因：</p>
  <pre>第六個樞紐在第 b 根，型態在 b+1 根確認
它的視窗 2 判定需要 b+2 根  ->  確認當下那一根還不存在</pre>
  <p class="note">Python 離線掃全序列，順手用了 <code>b+2</code>；指標只能用當下有的。
  <b>前瞻在參考實作那一側，不在指標。</b></p>
  <table>
    <tr><th>最後樞紐是視窗 2 樞紐</th><th>兩者差距</th><th class="n">筆數</th></tr>
    <tr><td>是</td><td>Python 多 1</td><td class="n">7</td></tr>
    <tr><td>否</td><td>相同</td><td class="n">4</td></tr>
    <tr><td colspan="2"><b>假說命中</b></td><td class="n ok"><b>11 / 11</b></td></tr>
  </table>
  <p class="note"><b>已修 Python 端。</b><code>s1</code> 範圍從 0-6 改為 <b>0-5</b>，
  平均從 4.14/6（含前瞻）改為 <b>3.32/5</b>。修正後 10/10 逐欄位相符。</p>
  <p class="note"><span class="tag">教訓</span>
  <b>指標不只是移植的對象，它也是參考實作的檢查器。</b>
  離線掃描能看到整條序列，逐根執行的程式碼不能 ——
  兩者對不上時，先問「是不是參考用了未來」。</p>
</div>
