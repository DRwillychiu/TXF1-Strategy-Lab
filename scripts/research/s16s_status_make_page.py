...
TOP = ('<div class="wrap"><header class="top">'
       ...
       '<h1>型態層總覽</h1>'
       '<p class="sub">純圖形型態 <b>72 種</b>：已處理 <b>57</b>，尚未討論 <b>15</b>。'
       '狀態讀自圖鑑清單並與結案文件交叉查核 —— <b>24 個結案全部有文件佐證</b>。</p>'
       '<div class="kpi">'
       '<span><b>33</b>已編碼並檢定</span>'
       '<span><b>24</b>已結案（含文件）</span>'
       '<span><b>15</b>尚未討論</span>'
       '<span><b>3</b>空方通過並進指標</span>'
       '</div></header>')

P1 = ('<section class="panel"><div class="ph2"><h2>一、已經處理完的 57 種</h2>'
      '<span class="tag">每一項都有結案文件</span></div>'
      '<div class="done">' + done + '</div></section>')

P2 = ('<section class="panel"><div class="ph2">'
      '<h2>二、尚未討論的 15 種 —— 依「共用什麼機制」分組</h2>'
...
print('  分組合計 %d 種（應為 15）%s' % (n, 'OK' if n == 15 else '★ 不符'))
d = sum(x[1] for x in DONE)
print('  結案合計 %d 種（應為 57）%s' % (d, 'OK' if d == 57 else '★ 不符'))
print('  57 + 15 = %d（應為 72）%s' % (d + n, 'OK' if d + n == 72 else '★ 不符'))
