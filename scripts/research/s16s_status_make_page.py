...
GROUPS = [
    ('A', '樞紐型 —— 可直接沿用既有框架', 7, 'ok',
     [('P15', '頭肩頂'), ('P16', '頭肩底'), ('P18', '三重頂'), ('P21', '上升通道'),
      ('P23', '三重底'), ('P57', '測量移動（下）'), ('P68', '雙頂四變體')],
     '樞紐鏈、交替、滑動取窗、M7 前段趨勢<b>全部已經寫好了</b>。'
     '這組的邊際成本最低，<b>建議一次做完七個</b>，就像擴散家族與缺口組那樣。'
     '要先裁示的只有兩件事：頭肩的「肩要幾個點」、三重頂的「三高相等」。'),
    ('B', '曲率型 —— 圓弧、杯、V', 8, 'warn',
    ...
    ('C', '幅度／速度型 —— 旗形、駝峰、塔形', 6, 'warn',
     [('P24', '多方旗形'), ('P25', '多方三角旗'), ('P55', '駝峰反轉'),
      ('P56', '死貓反彈'), ('P60', '高位緊密旗形'), ('P65', '塔形頂')],
     '「急漲」「緊密」「大幅」都需要幅度或速度門檻。'
     '旗形的旗桿<b>可以用斜率閘門從已固定的東西推導</b>（圖鑑已有前例），'
     '所以這組不是全無希望，但要逐一判斷。'),
    ('D', '本質上是參數 —— 建議直接結案', 5, 'bad',
     [('P07', '布林帶寬擠壓'), ('P58', '扇形原則'), ('P69', '艾略特 5 波'),
      ('P70', '費波那契回撤'), ('P71', '江恩角度線')],
     'P07 是指標型態不是純圖形（週期＋標準差兩個參數）；'
     'P58 三條趨勢線每條都是選擇；P69 是方法論不是型態；'
     'P70 的比例是<b>外來常數</b>；P71 的角度<b>依賴座標軸縮放</b>，在 5 分 K 上無定義。'
     '<b>原本的 D 組是 4 種，P58 應併入 —— 砍掉這 5 種，剩餘 27 → 22。</b>'),
    ('E', '其他', 1, 'warn', [('P72', '訂單塊 ／ 反轉塊')],
     '需要先定義「訂單塊」的零參數版本。可與 A 組一起量。'),
]
...
TOP = ('<div class="wrap"><header class="top">'
       '<div class="eyebrow">S16_S 型態層 · 總覽 · 2026-08-28</div>'
       '<h1>型態層總覽</h1>'
       '<p class="sub">純圖形型態 <b>72 種</b>：已處理 <b>45</b>，尚未討論 <b>27</b>。'
       '狀態讀自圖鑑清單並與結案文件交叉查核 —— <b>12 個結案全部有文件佐證</b>。</p>'
       '<div class="kpi">'
       '<span><b>33</b>已編碼並檢定</span>'
       '<span><b>12</b>已結案（含文件）</span>'
       '<span><b>27</b>尚未討論</span>'
       '<span><b>22</b>砍掉 D 組後的實際待辦</span>'
       '</div></header>')

P1 = ('<section class="panel"><div class="ph2"><h2>一、已經處理完的 45 種</h2>'
      '<span class="tag">每一項都有結案文件</span></div>'
      '<div class="done">' + done + '</div></section>')

P2 = ('<section class="panel"><div class="ph2">'
      '<h2>二、尚未討論的 27 種 —— 依「共用什麼機制」分組</h2>'
      '<span class="tag">綠＝便宜　橘＝先量母體　紅＝建議結案</span></div>'
...
P3 = ('<section class="panel"><div class="ph2"><h2>三、速度正在加快</h2>'
      ...
      '<div class="big"><p>最省時間的做法：<b>先把 27 種的母體一次量完</b>，'
      '再只對站得住的做深度討論。</p>'
...
P4 = ('<section class="panel"><div class="ph2"><h2>四、待裁示（三題，都不擋路）</h2>'
      ...
      '</tr></thead><tbody>'
      '<tr><td>D 組結案</td><td style="text-align:left">'
      '<b>砍掉 5 種</b>（原 4 種 ＋ P58 扇形原則），27 → 22</td></tr>'
...
print('  分組合計 %d 種（應為 27）%s' % (n, 'OK' if n == 27 else '★ 不符'))
