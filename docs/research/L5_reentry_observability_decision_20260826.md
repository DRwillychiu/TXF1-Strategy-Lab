...
中發生 1 次。2026-08-26 MC12 實跑確認：171 筆成交中有 1 筆
報表顯示 `Stop Loss`，找不到對應的 `Sell` 標籤。
...
- `scripts/research/join_l5_entry_log.py` 的報表解析器對真實報表
  178/178 通過四項（該次用的是暫存區一份較舊的 178 筆匯出）；
  合成 log 端對端 178/178 join、0 未匹配

### 2026-08-26 MC12 退化錨點結果（已跑）

使用者在 MC12 跑完 v199_R1 與 live，兩份報告逐格比對：
...
| 交易分析 | 595 | **0** |
| 週期性分析 | 2,491 | **0** |
| 設定 | 21 | 2（策略名稱、input 由 23 個變 24 個） |

淨利 1,505,000／交易 171 筆／勝率 56.1404%／獲利因子 1.2865／
最大策略虧損 −1,020,000（−22.7526%）——全部逐位數相同。

**編譯通過、跑得動、行為未變。** `v_is_in_consolidation[1]` 在 MC12
的行為因此也間接證實（若不合法會編譯失敗）。

基準數字更新：本次為 **171 筆**，非先前引用的 178 筆。178 來自暫存區
一份較舊的匯出，資料區間不同。往後 L5 以這兩份報告為準。

未驗證（不可宣稱）：
- **`Print` 是否真的印出東西，報表看不出來。** 需要 MC 輸出視窗內容。
